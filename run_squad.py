        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    #####for maintaining memory, delete previous checkpoints
                    delete_dir = os.path.join(args.output_dir, "checkpoint-{}".format(global_step-args.save_steps))
                    # 첫번째는 저장해
                    # if os.path.exists(delete_dir) and global_step==args.save_steps:
...
def evaluate(args, model, tokenizer, global_step=None):
    ...
    for batch in progress_bar(eval_dataloader):
        ...
        for i, example_index in enumerate(example_indices):
            ...
            # print(argmax(result.start_logits))
            # print(argmax(result.end_logits))
    ...
    with open(output_eval_file, "w", encoding='utf-8') as f:
        official_eval_results = eval_during_train_v2(args, step=global_step)
        results.update(official_eval_results)
    return results,output_eval_file
    # return None
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    cached_features_file = os.path.join(
        input_dir,
        #EOFError : Ran out of input -> erase _new
        "cached_{}_{}_{}_new_til_17".format(
    ...
    if os.path.exists(cached_features_file):
        ...
        if not args.data_dir and ((evaluate and not args.predict_file) or (not evaluate and not args.train_file)):
            ...
            if evaluate:
                ...
                ##filename = args.train_file ("KorQuAD/train/new_korquad2.1_train_00.json")
                for i in range(1,18):
                    if i<10:
                        tempname = ("KorQuAD/train/new_korquad2.1_train_0{}.json".format(i))
                    else:
                        tempname = ("KorQuAD/train/new_korquad2.1_train_{}.json".format(i))
                    new_examples = processor.get_train_examples(os.path.join(args.data_dir, args.task),
                                                        filename=tempname)
                    print(len(new_examples),"------------------------{}".format(i))
...
def main(cli_args):
    ...
    if args.do_train:
        ...
        ###########################
        print("length of train_data set is : " , len(train_dataset))
        #############################
    ...
    if args.do_eval:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result ,output_eval_file= evaluate(args, model, tokenizer, global_step=global_step)
